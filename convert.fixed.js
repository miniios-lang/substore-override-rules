...
- threshold: 地区节点数量小于该值时不显示分组 (默认 2；低于该数量的地区会被整组丢弃，进而可能让 自动选择/故障转移 变空)
...
(() => {
  ...
  function buildBaseLists({
    ...
    // 修复：落地组 + 地区组都为空时，退回真实节点，避免 自动选择/故障转移 拿到空数组
    const hasFallbackGroup = Boolean(landing) || suffixedCountryNames.length > 0;
    const defaultFallback = hasFallbackGroup ? buildList(landing && PROXY_GROUPS.LANDING, suffixedCountryNames) : buildList(fallbackNodeNames);
  ...
  // src/sanitize.ts
  // 统一安全网：保证输出的每个策略组都不会出现
  //   (a) proxies 为空            -> mihomo 报 "`use` or `proxies` missing"
  //   (b) 引用了不存在的名字      -> mihomo 报 "'xxx' not found"
  var BUILTIN_PROXY_NAMES = ["DIRECT", "REJECT", "REJECT-DROP", "COMPATIBLE", "PASS", "PASS-RULE"];
  function ensureUsableProxyGroups(groups, nodeNames) {
    const nodes = (nodeNames || []).filter(Boolean);
    const validNames = new Set([...BUILTIN_PROXY_NAMES, ...nodes]);
    for (const group of groups) {
      if (group && group.name) validNames.add(group.name);
    }
    const emergency = nodes.length > 0 ? [...nodes] : ["DIRECT"];
    return groups.map((group) => {
      if (!group || !group.name) return group;
      const isIncludeAll = group["include-all"] === true;
      if (!Array.isArray(group.proxies)) {
        return isIncludeAll ? group : { ...group, proxies: [...emergency] };
      }
      const cleaned = [...new Set(group.proxies)].filter(
        (name) => typeof name === "string" && validNames.has(name)
      );
      if (cleaned.length === 0 && !isIncludeAll) {
        return { ...group, proxies: [...emergency] };
      }
      return { ...group, proxies: cleaned };
    });
  }
    ...
    "src/main.ts"() {
      ...
      function main(config) {
        ...
        const {
          ...
          nonLandingNodes,
          regexFilter,
          fallbackNodeNames: (landing ? nonLandingNodes : config.proxies).map((node) => node.name).filter(Boolean)
        });
        ...
        const safeProxyGroups = ensureUsableProxyGroups(proxyGroups, allNodes);
